...
lmcirun <- function(x){
  ...
  keywords <- read_excel("staff_agencies.xlsx" )
  lengtha <- sum(!is.na(keywords$staff_agency_name))
  aword <- as.character(keywords$staff_agency_name[1:lengtha])
  filteredout <- filter(dframe, str_detect(dframe$companyname, paste(aword, collapse = '|')))
  dframe <- mutate(dframe, companyname = replace(companyname, str_detect(dframe$companyname, paste(aword, collapse = '|')), NA))
